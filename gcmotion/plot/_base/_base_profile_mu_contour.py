...
def _base_profile_pzeta_contour(profile: Profile, ax: Axes, **kwargs):
    ...
    zetalim = profile.Q(config.zetalim, "radians")
    psilim = profile.Q(config.psilim, "psi_wall").to(config.flux_units)
    psigrid, zetagrid = np.meshgrid(
        np.linspace(psilim[0], psilim[1], config.grid_density),
        np.linspace(zetalim[0], zetalim[1], config.grid_density),
    )
    ...
    if config.mode == "lines":
        C = ax.contour("zeta", "flux", "Pzeta", data=data, **kw)
    else:
        C = ax.contourf("zeta", "flux", "Pzeta", data=data, **kw)

    # Setup labels.
    # Also add a second axis for Ptheta
    ax.set_ylabel(rf"$\psi$ [{psigrid.units:.4g~P}]", size=config.labelsize)
    ax.set_xlabel(r"$\zeta$ [radians]", size=config.labelsize)
    ...
    if config.wall:
        rect = Rectangle(
            ...
            width=abs(zetalim[1].m - zetalim[0].m),
    ...
    if config.Pthetaax and config.cursor:
        ...
        def cursor_format(x, y):
            ...
            return (
                ...
                + f"Pzeta = {z:.4g} {config.Pzeta_units}"
            )
